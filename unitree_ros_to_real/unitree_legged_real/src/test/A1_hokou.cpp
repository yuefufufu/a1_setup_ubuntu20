...
int mainHelper(int argc, char *argv[], TLCM &roslcm)
{
    ...
    while (ros::ok()){
        ...
        if(flag == 1 && 0 <= count && count < Hz[1]){
            ...
            x_wave = -1 * cos(M_PI * ((count - Hz[1] - Hz[2])/Hz[1]));
        ...
        if(flag == 1){
            FRb_x = x_ofs + ((x_int + p5) * x_wave);
            FLb_x = x_ofs - ((x_int - p5) * x_wave);
            RRb_x = x_ofs - ((x_int + p5) * x_wave);
            RLb_x = x_ofs + ((x_int - p5) * x_wave);
            FRb_y = y_ofs + 0.03 + ((y_int + p6) * y_wave);
            FLb_y = -1 * y_ofs - 0.03 - ((y_int + p6) * y_wave);
            RRb_y = y_ofs + 0.03 - ((y_int - p6) * y_wave);
            RLb_y = -1 * y_ofs - 0.03 + ((y_int - p6) * y_wave);
        }
    ...
    {    
        ...
        if(FR_q[0] > 0.8){FR_q[0] = 0.8;ROS_WARN("Angle command value is out of range");}
        if(FR_q[0] < -0.8){FR_q[0] = -0.8;ROS_WARN("Angle command value is out of range");}
        if(FL_q[0] > 0.8){FL_q[0] = 0.8;ROS_WARN("Angle command value is out of range");}
        if(FL_q[0] < -0.8){FL_q[0] = -0.8;ROS_WARN("Angle command value is out of range");}
        if(RR_q[0] > 0.8){RR_q[0] = 0.8;ROS_WARN("Angle command value is out of range");}
        if(RR_q[0] < -0.8){RR_q[0] = -0.8;ROS_WARN("Angle command value is out of range");}
        if(RL_q[0] > 0.8){RL_q[0] = 0.8;ROS_WARN("Angle command value is out of range");}
        if(RL_q[0] < -0.8){RL_q[0] = -0.8;ROS_WARN("Angle command value is out of range");}

        if(FR_q[1] > 4.0){FR_q[1] = 4.0;ROS_WARN("Angle command value is out of range");}
        if(FR_q[1] < -1.0){FR_q[1] = -1.0;ROS_WARN("Angle command value is out of range");}
        if(FL_q[1] > 4.0){FL_q[1] = 4.0;ROS_WARN("Angle command value is out of range");}
        if(FL_q[1] < -1.0){FL_q[1] = -1.0;ROS_WARN("Angle command value is out of range");}
        if(RR_q[1] > 4.0){RR_q[1] = 4.0;ROS_WARN("Angle command value is out of range");}
        if(RR_q[1] < -1.0){RR_q[1] = -1.0;ROS_WARN("Angle command value is out of range");}
        if(RL_q[1] > 4.0){RL_q[1] = 4.0;ROS_WARN("Angle command value is out of range");}
        if(RL_q[1] < -1.0){RL_q[1] = -1.0;ROS_WARN("Angle command value is out of range");}
        
        if(FR_q[2] > -1.0){FR_q[2] = -1.0;ROS_WARN("Angle command value is out of range");}
        if(FR_q[2] < -2.6){FR_q[2] = -2.6;ROS_WARN("Angle command value is out of range");}
        if(FL_q[2] > -1.0){FL_q[2] = -1.0;ROS_WARN("Angle command value is out of range");}
        if(FL_q[2] < -2.6){FL_q[2] = -2.6;ROS_WARN("Angle command value is out of range");}
        if(RR_q[2] > -1.0){RR_q[2] = -1.0;ROS_WARN("Angle command value is out of range");}
        if(RR_q[2] < -2.6){RR_q[2] = -2.6;ROS_WARN("Angle command value is out of range");}
        if(RL_q[2] > -1.0){RL_q[2] = -1.0;ROS_WARN("Angle command value is out of range");}
        if(RL_q[2] < -2.6){RL_q[2] = -2.6;ROS_WARN("Angle command value is out of range");}
    }
